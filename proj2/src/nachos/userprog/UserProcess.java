...
public class UserProcess {
	...
	private int handleExec(int file, int argc, int argv){
		...
		if(child.execute(fileName, args)){
			child.parent=this;
			map.put(child.processId, new childProcess(child));
	...
	private void handleExit(int status){
		...
		if(parentPtr!=null){
			parentPtr.map.get(this.processId).status=status;
			this.parent=null;
		}
		...
		//close all the opened files, todo
		...
		public static int processIdGenerator=0;
